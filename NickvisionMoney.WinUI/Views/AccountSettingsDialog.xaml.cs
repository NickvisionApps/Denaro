        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var regex = $@"^(?:{emojiPattern})+\z";
                    if (Regex.Match(split[0], regex).Success && Regex.Match(split[1], regex).Success)
                    {
                        LblId.Text = $"{split[0][0]}{split[0][1]}{split[1][0]}{split[1][1]}";
                    }
                    else if (Regex.Match(split[0], regex).Success)
                    {
                        LblId.Text = $"{split[0][0]}{split[0][1]}{split[1][0]}";
                    }
                    else if (Regex.Match(split[1], regex).Success)
                    {
