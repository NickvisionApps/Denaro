...
Application::Application(const std::string& id, GApplicationFlags flags) : m_adwApp{ adw_application_new(id.c_str(), flags) }
{
    ...
    m_appInfo.setChangelog("<ul><li>Introducing Groups: Add groups to an account and associate transactions with groups for a more precise finance management system</li><li>Added translation support</li><li>Added French translation (Thanks @zothma!)</li><li>Added Hindi translation (Thanks @securearth!)</li><li>Added Italian translation (Thanks @bordam!)</li><li>Added Polish translation (Thanks @martin-desktops!)</li><li>Added Russian translation (Thanks @fsobolev!)</li></ul>");
